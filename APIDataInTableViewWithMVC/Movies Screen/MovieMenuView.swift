...
//            cell.movieImageViewOutlet.sd_setImage(with: URL(string: "http://www.domain.com/path/to/image.jpg"), placeholderImage: UIImage(named: "placeholder.png"))
            let url = URL(string: movieList?.data?.data?[indexPath.row].imagery?.mobileMenuPosterImage ?? "")
            cell.movieImageViewOutlet.sd_setImage(with: url, completed: nil)
            cell.movieNameLabelOutlet.text = movieList?.data?.data?[indexPath.row].title ?? ""
//            cell.configureUI(image: , title: <#T##String#>)
            return cell
        }
